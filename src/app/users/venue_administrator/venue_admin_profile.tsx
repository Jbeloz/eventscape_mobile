...
export default function VenueAdminProfile() {
  ...
  const loadUserData = async () => {
    ...
    try {
      // Load user phone number and photo using email (not auth_id)
  ...
  const uploadAndSavePhoto = async (imageUri: string) => {
    ...
    setUploading(true);
    try {
      // First, get the actual user_id from the database using email (not auth_id)
      const { data: userData } = await supabase
        .from("users")
        .select("user_id")
        .eq("email", user.email.toLowerCase())
      ...
      // Delete old photo if exists, then insert new one
      try {
        await supabase
          .from("user_photos")
          .delete()
        ...
        console.warn("Note: Could not delete old photo (might not exist):", deleteError);
      }
      ...
      console.log("Photo record inserted:", insertData);
  ...
  const handleSaveProfile = async () => {
    ...
    setSaving(true);
    try {
      // Update in Supabase users table using email (not auth_id)
      if (user?.email) {
